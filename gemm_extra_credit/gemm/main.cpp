...
int main(int argc, char *argv[]) {
...
#if MKL_INSTALLED
  double minMKL = 1e30;
  double totalsqerr_ispc =
      0;  // keep track of total squared error over all iterations
#endif
  double minGEMM = 1e30;
  double minISPC = 1e30;
  double totalsqerr_user =
      0;  // keep track of total squared error over all iterations
  ...
  for (int i = 0; i < N_ITERS; ++i) {
    ...
    cblas_dgemm(CblasRowMajor, CblasNoTrans, CblasNoTrans, m, n, k, alpha, A1,
                k, B1, n, beta, C1, n);
    ...
    for (int i = 0; i < m; i++) {
      for (int j = 0; j < n; j++) {
        ...
        totalsqerr_user +=
            (mkl_output - sol_output) * (mkl_output - sol_output);
        totalsqerr_ispc +=
            (mkl_output - ispc_output) * (mkl_output - ispc_output);
#else
        double sol_output  = C2[i * n + j];
        double ispc_output = C3[i * n + j];
        totalsqerr_user +=
            (ispc_output - sol_output) * (ispc_output - sol_output);
#endif
      }
    }
  }
  ...
  printf("[Intel MKL]:\t\t[%.3f] ms\t[%.3f] GB/s\t[%.2f] GFLOPS\n",
         minMKL * 1000, toBW(TOTAL_BYTES, minMKL),

...
void usage(const char *progname, std::string *testnames, int num_tests) {
  ...
  printf(
      "  -n  --num_threads  <INT>      Number of threads: <INT> (default=%d)\n",
      DEFAULT_NUM_THREADS);
  printf(
      "  -i  --num_timing_iterations <INT> Number of timing iterations: <INT> "
      "(default=%d)\n",
      DEFAULT_NUM_TIMING_ITERATIONS);
  printf("  -?  --help                    This message\n");
  printf("Valid testnames are:");
  for (int i = 0; i < num_tests; i++) {
    printf(" %s%c", testnames[i].c_str(),
           (char)((i + 1 == num_tests) ? '\n' : ','));
  }
}
...
ITaskSystem *selectTaskSystemRefImpl(int num_threads, TaskSystemType type) {
  ...
  if (type == SERIAL) {
    ...
    return new TaskSystemParallelThreadPool(num_threads, true, true);
  } else if (type == PARALLEL_THREAD_POOL_SLEEPING) {
    return new TaskSystemParallelThreadPool(num_threads, false, false);
  } else {
    return NULL;
  }
}
  ...
  for (int test_id = 0; test_id < n_tests; test_id++) {
    ...
    for (int i = 0; i < N_TASKSYS_IMPLS; i++) {
      ...
      for (int j = 0; j < num_timing_iterations; j++) {
        ...
        ITaskSystem *t =
            selectTaskSystemRefImpl(num_threads, (TaskSystemType)i);
        ...
        if (!result.passed) {
          printf(
              "ERROR: Results did not pass correctness check! (iter=%d, "
              "ref_impl=%s)\n",
              j, t->name());
          exit(1);
        }

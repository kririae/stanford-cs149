...
class LightTask : public IRunnable {
 public:
  ...
  void runTask(int task_id, int num_total_tasks) { output_[task_id] = task_id; }
};
...
class ReduceTask : public IRunnable {
 public:
  ...
  int    array_size_;
  int    num_to_reduce_;
  ReduceTask(int array_size, int num_to_reduce, float* input, float* output) {
    array_size_    = array_size;
...
class MandelbrotTask : public IRunnable {
 public:
  ...
  void mandelbrotSerial_interleaved(float x0, float y0, float x1, float y1,
                                    int width, int height, int startRow,
                                    int totalRows, int interleaving,
                                    int max_iterations, int output[]) {
    float dx = (x1 - x0) / width;
    float dy = (y1 - y0) / height;
  ...
  void runTask(int task_id, int num_total_tasks) {
    ...
    if (interleave_ == 1) {
      mandelbrotSerial_interleaved(args_->x0, args_->y0, args_->x1, args_->y1,
                                   args_->width, args_->height, task_id,
                                   args_->height - task_id, num_total_tasks,
                                   args_->max_iterations, args_->output);
    } else {
      mandelbrotSerial(args_->x0, args_->y0, args_->x1, args_->y1, args_->width,
                       args_->height, task_id * rowsPerTask, rowsPerTask,
                       args_->max_iterations, args_->output);
    }
  }
};
...
class SleepTask : public IRunnable {
 public:
  int sleep_period_;
  SleepTask(int sleep_period) { sleep_period_ = sleep_period; }
  ~SleepTask() {}

  void runTask(int task_id, int num_total_tasks) {
    std::this_thread::sleep_for(std::chrono::seconds(sleep_period_));
 ...
 public:
  ...
  void doWork(int task_id, int num_total_tasks) {
    ...
    std::this_thread::sleep_for(
        std::chrono::microseconds((1 + (task_id % 10))));
  }
...
TestResults pingPongTest(ITaskSystem* t, bool equal_work, bool do_async,
  ...
  delete[] input;
  delete[] output;
  for (int i = 0; i < num_bulk_task_launches; i++) delete runnables[i];
 ...
 * in a tight for loop. Tasks are sufficiently compute-intensive and
 * lightweight: the threadpool implementation should perform better than the one
 * that spawns new threads with every bulk task launch.
 */
TestResults mathOperationsInTightForLoopTestBase(ITaskSystem* t, int num_tasks,
                                                 bool run_with_dependencies,
                                                 bool do_async) {
 ...
 */
TestResults mathOperationsInTightForLoopFanInTestBase(ITaskSystem* t,
                                                      bool         do_async) {
  int num_tasks              = 64;
  ...
  if (do_async) {
    ...
    for (int i = 0; i < num_bulk_task_launches; i++) {
      TaskID task_id =
          t->runAsyncWithDeps(&medium_tasks[i], num_tasks, no_deps);
  ...
  for (int i = 0; i < array_size; i++) {
    if (i % 3 == 0) {
      if (std::floor(final_task_output[i]) != 89577) {
        printf("%d: %f expected=%d\n", i, std::floor(final_task_output[i]),
               69982);
        result.passed = false;
      }
    } else if (i % 3 == 1) {
      ...
      if (std::floor(final_task_output[i]) !=
 ...
 * computation DAG.
 */
TestResults mathOperationsInTightForLoopReductionTreeTestBase(ITaskSystem* t,
                                                              bool do_async) {
  int num_tasks              = 64;
  ...
  std::vector<MathOperationsInTightForLoopTask> medium_tasks;
  for (int i = 0; i < num_bulk_task_launches; i++) {
    medium_tasks.push_back(
        MathOperationsInTightForLoopTask(array_size, &buffer1[i * array_size]));
  }
  ...
  while (num_reduce_tasks >= 1) {
    ...
    for (int i = 0; i < num_reduce_tasks; i++) {
      ReduceTask reduce_task(array_size, 2, input, output);
  ...
  if (do_async) {
    ...
    while (num_reduce_tasks >= 1) {
      for (int i = 0; i < num_reduce_tasks; i++) {
        TaskID task_id =
            t->runAsyncWithDeps(&reduce_tasks[reduce_idx + i], 1, all_deps[i]);
        cur_deps.push_back(task_id);
        if (cur_deps.size() == 2) {
  ...
  for (int i = 0; i < array_size; i++) {
    if (i % 3 == 0) {
      if (std::floor(buffer6[i]) != 11197) {
        printf("%d: %f expected=%d\n", i, std::floor(buffer6[i]), 11197);
        result.passed = false;
      }
    } else if (i % 3 == 1) {
      if (std::floor(buffer6[i]) != 22687) {
        printf("%d: %f expected=%d\n", i, std::floor(buffer6[i]), 22687);
        result.passed = false;
      }
    } else {
      if (std::floor(buffer6[i]) != (67950 * num_bulk_task_launches)) {
        printf("%d: %f expected=%d\n", i, std::floor(buffer6[i]),
               67950 * num_bulk_task_launches);
        result.passed = false;
      }
    }
...
TestResults spinBetweenRunCallsTestBase(ITaskSystem* t, bool do_async) {
  ...
  if (do_async) {
    ...
    TaskID medium_task_id =
        t->runAsyncWithDeps(&medium_task, num_med_tasks, deps);
    deps.push_back(light_task_id);
    deps.push_back(medium_task_id);
    t->runAsyncWithDeps(&light_task, num_light_tasks, deps);
    t->sync();
  } else {
...
TestResults mandelbrotChunkedTestBase(ITaskSystem* t, bool do_async) {
  ...
  mandel_task.mandelbrotSerial(ma.x0, ma.y0, ma.x1, ma.y1, ma.width, ma.height,
                               0, ma.height, ma.max_iterations, golden);
 ...
 */
TestResults strictGraphDepsTestBase(ITaskSystem* t, int n, int m,
                                    unsigned int seed) {
  // For repeatability.
  srand(seed);
  ...
  for (int i = 0; i < n; i++) {
    ...
    task_ids[i] =
        t->runAsyncWithDeps(tasks[i], (rand() % 15) + 1, task_deps[i]);
  }
  t->sync();
  double end_time = CycleTimer::currentSeconds();

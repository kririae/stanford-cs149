...
void usage(const char* progname) {
  ...
  printf(
      "  -m  --test <TYPE>      Run specified function on input.  Valid tests "
      "are: scan, find_repeats (default: scan)\n");
  printf(
      "  -i  --input <NAME>     Run test on given input type. Valid inputs "
      "are: ones, random (default: random)\n");
...
int main(int argc, char** argv) {
  ...
  static struct option long_options[] = {
      {"test", 1, 0, 'm'}, {"arraysize", 1, 0, 'n'}, {"input", 1, 0, 'i'},
      {"help", 0, 0, '?'}, {"thrust", 0, 0, 't'},    {0, 0, 0, 0}};

  while ((opt = getopt_long(argc, argv, "m:n:i:?t", long_options, NULL)) !=
         EOF) {
    switch (opt) {
      case 'm':
        test = optarg;
    ...
    for (int i = 0; i < 1; i++) {
      ...
      if (useThrust)
        cudaTime = std::min(cudaTime,
                            cudaScanThrust(inarray, inarray + N, resultarray));
      else
        cudaTime =
            std::min(cudaTime, cudaScan(inarray, inarray + N, resultarray));
    }

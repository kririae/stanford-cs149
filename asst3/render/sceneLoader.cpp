...
static void makeCircleGrid(int startIndex, int circleCount, float circleRadius,
                           float circleColor[3], float startOffsetX,
                           float startOffsetY, float* position, float* color,
                           float* radius) {
...
static void generateRandomCircles(int numCircles, float* position,
                                  float* velocity, float* color,
                                  float* radius) {
  srand(0);
...
static void generateSizeCircles(int numCircles, float* position,
  ...
  for (int i = 0; i < numCircles; i++) {
    ...
    position[index3] =
        randomFloat();  // targetR + (1.f - targetR) * randomFloat();
    position[index3 + 1] =
        randomFloat();  // targetR + (1.f - targetR) * randomFloat();
    position[index3 + 2] = depth;
  ...
  if (sceneName == SNOWFLAKES) {
    ...
    for (int i = 0; i < numCircles; i++) {
      ...
      if (fscanf(file, "%f %f %f   %f %f %f   %f\n", &position[index3],
                 &position[index3 + 1], &position[index3 + 2],
                 &velocity[index3], &velocity[index3 + 1],
                 &velocity[index3 + 2], &radius[i]) != 7) {
    ...
    for (int i = 0; i < numCircles; i++) radius[i] = .3f;
    ...
    // test scene with many big circles and one tile at the bottom right having
    // many small circles
    numCircles = 10 * 1000;
    ...
    generateSizeCircles(numCircles, position, velocity, color, radius,
                        BIG_RADIUS);
    int startIdx  = 9 * 1000;
    int changeNum = 1 * 1000;
    changeCircles(changeNum, (position + startIdx * 3), (radius + startIdx),
                  MICRO_RADIUS, .85f, .1f);
    ...
    // test scene with many big circles and one tile at the top left having many
    // big circles
    numCircles = 10 * 1000;
    ...
    circleColor[1] = 0.f;
    circleColor[2] = 0.f;
    makeCircleGrid(startIndex, circleCount1, circleRadius, circleColor,
                   startOffsetX, startOffsetY, position, color, radius);
    ...
    circleColor[1] = 1.f;
    circleColor[2] = .0f;
    makeCircleGrid(startIndex, circleCount2, circleRadius, circleColor,
                   startOffsetX, startOffsetY, position, color, radius);
  } else {

...
static void compare_images(const Image* ref_image, const Image* cuda_image) {
  ...
  if (ref_image->width != cuda_image->width ||
      ref_image->height != cuda_image->height) {
  ...
  for (i = 0; i < 4 * ref_image->width * ref_image->height; i++) {
    ...
    if (fabs(ref_image->data[i] - cuda_image->data[i]) > 0.1f && i % 4 != 3) {
      ...
      printf("Mismatch detected at pixel [%d][%d], value = %f, expected %f ",
             j / cuda_image->width, j % cuda_image->width, cuda_image->data[i],
             ref_image->data[i]);
...
void startBenchmark(CircleRenderer* renderer, int startFrame, int totalFrames,
  ...
  for (int frame = 0; frame < startFrame + totalFrames; frame++) {
    if (frame == startFrame) startTime = CycleTimer::currentSeconds();
  ...
  printf("Total:    %.4f ms\n",
         1000.f * (totalClearTime + totalAdvanceTime + totalRenderTime) /
             totalFrames);
  if (dumpFrames)
...
void CheckBenchmark(CircleRenderer* ref_renderer, CircleRenderer* cuda_renderer,
                    int startFrame, int totalFrames,
                    const std::string& frameFilename) {
  double totalClearTime    = 0.f;
  double totalAdvanceTime  = 0.f;
  double totalRenderTime   = 0.f;

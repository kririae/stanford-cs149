...
void usage(const char* progname) {
  ...
  printf(
      "  -r  --renderer <cpuref/cuda>  Select renderer: ref or cuda "
      "(default=cuda)\n");
  printf(
      "  -s  --size  <INT>             Rendered image size: <INT>x<INT> pixels "
      "(default=%d)\n",
      DEFAULT_IMAGE_SIZE);
  printf(
      "  -b  --bench <START:END>       Run for frames [START,END) "
      "(default=[0,1))\n");
  printf(
      "  -c  --check                   Check correctness of CUDA output "

...
double compute_score(std::string graph_name, bool correct, double ref_time,
                     double stu_time) {
...
void run_on_graph(int idx, graph* g, int num_threads, int num_runs,
  ...
  scores[idx][top_down] =
      compute_score(graph_name, correct, ref_top_down_time, stu_top_down_time);
  ...
  scores[idx][bott_up] = compute_score(graph_name, correct, ref_bottom_up_time,
                                       stu_bottom_up_time);
  ...
  scores[idx][hybrid] =
      compute_score(graph_name, correct, ref_hybrid_time, stu_hybrid_time);
...
void print_scores(std::vector<std::string>         grade_graphs,
                  std::vector<std::vector<double>> scores) {
  ...
  for (int g = 0; g < grade_graphs.size(); g++) {
    ...
    bool small = false;
    if ((graph_name == "grid1000x1000.graph") ||
        (graph_name == "soc-livejournal1_68m.graph") ||

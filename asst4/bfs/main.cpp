...
int main(int argc, char** argv) {
  ...
  if (argc < 2) {
    ...
    std::cerr
        << "  To run results for all thread counts: <path/to/graph/file>\n";
  ...
  if (thread_count <= -1) {
    ...
    for (int i = 0; i < n_usage; i++) {
      ...
      for (int j = 0; j < g->num_nodes; j++) {
        if (sol1.distances[j] != sol4.distances[j]) {
          fprintf(stderr, "*** Results disagree at %d: %d, %d\n", j,
                  sol1.distances[j], sol4.distances[j]);
          tds_check = false;
          break;
        }
      }
      ...
      for (int j = 0; j < g->num_nodes; j++) {
        if (sol2.distances[j] != sol4.distances[j]) {
          fprintf(stderr, "*** Results disagree at %d: %d, %d\n", j,
                  sol2.distances[j], sol4.distances[j]);
          bus_check = false;
          break;
        }
      }
      ...
      for (int j = 0; j < g->num_nodes; j++) {
        if (sol3.distances[j] != sol4.distances[j]) {
          fprintf(stderr, "*** Results disagree at %d: %d, %d\n", j,
                  sol3.distances[j], sol4.distances[j]);
          hs_check = false;
          break;
        }
      }
      ...
      sprintf(buf, "%4d:    %.2f (%.2fx)      %.2f (%.2fx)      %.2f (%.2fx)\n",
              num_threads[i], top_time, top_base / top_time, bottom_time,
              bottom_base / bottom_time, hybrid_time,
              hybrid_base / hybrid_time);
      sprintf(ref_buf,
              "%4d:    %.2f (%.2fx)      %.2f (%.2fx)      %.2f (%.2fx)\n",
              num_threads[i], ref_top_time, ref_top_base / ref_top_time,
              ref_bottom_time, ref_bottom_base / ref_bottom_time,
              ref_hybrid_time, ref_hybrid_base / ref_hybrid_time);
      sprintf(relative_buf, "%4d:   %14.2f     %14.2f     %14.2f\n",
              num_threads[i], ref_top_time / top_time,
              ref_bottom_time / bottom_time, ref_hybrid_time / hybrid_time);
    ...
    if (!tds_check) std::cout << "Top Down Search is not Correct" << std::endl;
    if (!bus_check) std::cout << "Bottom Up Search is not Correct" << std::endl;
    if (!hs_check) std::cout << "Hybrid Search is not Correct" << std::endl;
    std::cout << std::endl
  ...
  // Run the code with only one thread count and only report speedup
    ...
    sprintf(ref_buf, "%4d:     %8.2f     %8.2f     %8.2f\n", thread_count,
            ref_top_time, ref_bottom_time, ref_hybrid_time);

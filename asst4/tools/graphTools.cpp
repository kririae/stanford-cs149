...
void print_help(const char* binary_name) {
  ...
  std::cerr << "Use '" << binary_name
            << " cmd' to get command-specific help.\n";
  std::cerr << "\n";
  std::cerr
      << "Valid cmds are:\n\n"
      ...
      << CMD_EDGESTATS
      << ": print stats on graph edges: e.g., min/max edges per node, etc.\n";
}
  ...
  if (!cmd.compare(CMD_TEXT2BIN)) {
    if (argc < 4) {
      std::cerr << "Usage: " << argv[0] << " " << cmd
                << " textfilename binfilename\n";
      std::cerr
          << "Converts a graph from text file format to binary file format\n";
      exit(1);
    }
    ...
    if (argc < 3) {
      std::cerr << "Usage: " << argv[0] << " " << cmd << " filename\n";
      std::cerr << "Pretty-prints graph, including edge information (be "
                   "careful with large graphs)\n";
      exit(1);
    }
    ...
    std::cout << "\n";
    std::cout << zero_outgoing.size() << " of " << num_nodes(g)
              << " nodes have zero outgoing edges (" << std::setprecision(2)
              << 100.0 * static_cast<double>(zero_outgoing.size()) /
                     num_nodes(g)
              << "\%).\n";
    delete g;
    ...
    std::cout << "\n";
    std::cout << zero_incoming.size() << " of " << num_nodes(g)
              << " nodes have zero incoming edges (" << std::setprecision(2)
              << 100.0 * static_cast<double>(zero_incoming.size()) /
                     num_nodes(g)
              << "\%).\n";
    delete g;
    ...
    for (int i = 0; i < num_nodes(g); i++) {
      ...
      for (const Vertex* v = out_begin; v != out_end; v++) {
        ...
        if (!found_symmetric) is_symmetric = false;
      }
    }
    ...
    std::cout << "=========================================================\n";
    std::cout << "The graph " << ((is_symmetric) ? "IS " : "IS NOT ")
              << "symmetric.\n";
    std::cout << "Outgoing edges: total=" << total_outgoing
              << " avg=" << avg_outgoing << " min=" << min_outgoing
              << " max=" << max_outgoing << "\n";

    std::cout << "Incoming edges: total=" << total_incoming
              << " avg=" << avg_incoming << " min=" << min_incoming
              << " max=" << max_incoming << "\n";
  }

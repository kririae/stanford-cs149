...
void clampedExpVector(float* values, int* exponents, float* output, int N) {
  ...
  __cs149_mask      maskAll, maskExpZero, maskExpNonZero, maskExceed;

  int i;
  for (i = 0; i < N; i += VECTOR_WIDTH) {
    int nbits = i + VECTOR_WIDTH > N ? N % VECTOR_WIDTH : VECTOR_WIDTH;
    maskAll   = _cs149_init_ones(nbits);

    _cs149_vload_float(x, values + i, maskAll);
    _cs149_vload_int(y, exponents + i, maskAll);

    _cs149_veq_int(maskExpZero, y, zero, maskAll);  // if (y == 0)
    _cs149_vgt_int(maskExpNonZero, y, zero, maskAll);
    _cs149_vmove_float(result, one_f, maskExpZero);  // result = x
    ...
    while (_cs149_cntbits(maskExpNonZero) > 0) {
      _cs149_veq_int(maskExpZero, count, zero, maskAll);
      _cs149_vgt_int(maskExpNonZero, count, zero, maskAll);
      _cs149_vmult_float(result, result, x, maskExpNonZero);
      _cs149_vsub_int(count, count, one, maskExpNonZero);
    }

    _cs149_vgt_float(maskExceed, result, upper_bound, maskAll);
    _cs149_vmove_float(result, upper_bound, maskExceed);  // result = x

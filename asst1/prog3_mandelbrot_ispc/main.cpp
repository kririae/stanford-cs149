...
int main(int argc, char** argv) {
  ...
  int                  opt;
  static struct option long_options[] = {{"tasks", 0, 0, 't'},
                                         {"view", 1, 0, 'v'},
                                         {"help", 0, 0, '?'},
                                         {0, 0, 0, 0}};
  ...
  for (unsigned int i = 0; i < width * height; ++i) output_serial[i] = 0;
  ...
  for (unsigned int i = 0; i < width * height; ++i) output_ispc[i] = 0;
  ...
  writePPMImage(output_ispc, width, height, "mandelbrot-ispc.ppm",
                maxIterations);
  ...
  if (useTasks) {
    ...
    writePPMImage(output_ispc_tasks, width, height, "mandelbrot-task-ispc.ppm",
                  maxIterations);
